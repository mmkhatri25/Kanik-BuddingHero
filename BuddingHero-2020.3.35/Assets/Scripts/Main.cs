...
public class Main : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		PlayerPrefs.SetInt("MaxStageReached", 350);
	...
	//	try
		...
		/*	if (IapManager.inst.boughtProductIndex == IapIds.XMAS_PACK)
			...
			else if (IapManager.inst.boughtProductIndex == IapIds.CANDY_PACK_01 ||
				IapManager.inst.boughtProductIndex == IapIds.CANDY_PACK_02)
			{
				PlayerStats.BoughtIAP(IapManager.inst.boughtProductIndex);
				IapManager.inst.boughtProductIndex = -1;
			...
			else if (IapManager.inst.boughtProductIndex == IapIds.CHRISTMAS_GEMS_BIG_PACK ||
				IapManager.inst.boughtProductIndex == IapIds.CHRISTMAS_GEMS_SMALL_PACK)
			{
			...
			else if (IapManager.inst.boughtProductIndex == IapIds.SECOND_ANNIVERSARY_GEMS ||
				IapManager.inst.boughtProductIndex == IapIds.SECOND_ANNIVERSARY_GEMS_TWO)
			{
			...
			else if (IapManager.inst.boughtProductIndex == IapIds.SECOND_ANNIVERSARY_CURRENCY_PACK ||
				IapManager.inst.boughtProductIndex == IapIds.SECOND_ANNIVERSARY_CURRENCY_PACK_TWO)
			{
		...
		if (this.taps.HasNone())
		{
			if (Input.GetMouseButtonDown(0))
			{
				...
				if (!flag3 && !GameMath.GetProbabilityOutcome(this.autoClickerDetection, GameMath.RandType.NoSeed))
				{
					...
					PlayerStats.OnTap();
					this.autoClickerDetection += 0.25f;
				}
			}
